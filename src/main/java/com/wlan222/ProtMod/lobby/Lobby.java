...
public class Lobby {
	...
	/**
	 *
	 * @param lm The LobbyManager of your Minigame
	 * @param gameid The unique Minigame ID
	 * @param lobbyid The unique id of this Lobby
	 * @param pl Your Plugin
	 */
	...
	/**
	 *
	 * @param p Player that should join
	 */
	public void join(Player p) {
		LobbyJoinEvent e = new LobbyJoinEvent(p, this);
		...
		players.add(p);
		lm.addPlayer(p, this);
		LobbyJoinedEvent lje = new LobbyJoinedEvent(p, this);
		Bukkit.getPluginManager().callEvent(lje);
	}

	/**
	 *
	 * @param p Player that should leave the Lobby
	 */
	public void leave(Player p) {
		LobbyLeaveEvent e = new LobbyLeaveEvent(p, this);
		Bukkit.getPluginManager().callEvent(e);
		players.remove(p);
		lm.removePlayer(p);
	}
	...
	/**
	 *
	 * @param s Message to be broadcasted
	 */
	 ...
	 *
	 * @param s Message to be broadcasted
	 * @param exclude A List of Players that should not receive this message
	 */
	 ...
	 *
	 * @param s Message to be broadcasted
	 * @param exclude A Player that should not receive the Message
	 */
	...
	public void stopGame() {
		gameRunning = false;
		GameEndEvent gee = new GameEndEvent(this);
	...
	public void cleanup() {
		if (isGameRunning()) {
			GameForceQuitEvent gfqe = new GameForceQuitEvent(this);
			Bukkit.getPluginManager().callEvent(gfqe);
		}
		for (Player p : players) {
			lm.removePlayer(p);
		}
		LobbyCleanupEvent lce = new LobbyCleanupEvent(this);
	...
	public void saveConfig() {
		...
		try {
			...
			pl.getLogger().log(Level.SEVERE, "Could not save config to " + cfgf, ex);
		}
