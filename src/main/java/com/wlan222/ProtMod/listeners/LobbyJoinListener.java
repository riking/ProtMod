...
public class LobbyJoinListener implements Listener {
	...
	public void onLobbyJoinEvent(LobbyJoinEvent event) {
		if (pl.isPlayer(event.getPlayer())) {
			if (event.getLobby().getGameID()
					.equals(pl.getLobbyByPlayer(event.getPlayer()).getGameID())) {
				event.getPlayer().sendMessage(
						ChatColor.RED + "You already joined another Lobby");
			} else {
				event.getPlayer().sendMessage(
						ChatColor.RED + "You are in a Lobby of " + pl.getLobbyByPlayer(event.getPlayer()).getGameID());
			}
